...
static void w25q128fv_deviceReset(flashDevice_t *fdevice)
{
    ...
    if (registerValue & W25Q128FV_SR2_BIT_QUAD_ENABLE) {
        // Don't enable QPI mode, otherwise *all* commands and addresses would have to be sent using 4 wires.
        // If QPI mode disabled commands can still be sent using 1,2 or 4 wires.
        // w25q128fv_performOneByteCommand(io, W25Q128FV_INSTRUCTION_ENTER_QPI_MODE);
    } else {
        // if the QE bit is not set now we can't send commands using 2/4 wires.
        // The flash API does not support reporting device reset failures.
        // 2/4 wire commands will just fail/timeout, which will be reported via the flash API.
    }
#endif
}
