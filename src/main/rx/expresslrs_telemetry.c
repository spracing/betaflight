...
{
    if (senderState != SENDER_IDLE || lengthToTransmit / bpc >= ELRS_TELEMETRY_MAX_PACKAGES) {
        return;
    }
...
void confirmCurrentTelemetryPayload(const bool telemetryConfirmValue)
{
    ...
    case WAIT_UNTIL_NEXT_CONFIRM:
        if (telemetryConfirmValue == waitUntilTelemetryConfirm) {
            ...
            waitUntilTelemetryConfirm = !telemetryConfirmValue;
        } else if (senderState == WAIT_UNTIL_NEXT_CONFIRM) { // switch to resync if tx does not confirm value fast enough
            waitCount++;
            if (waitCount > maxWaitCount) {
                waitUntilTelemetryConfirm = !telemetryConfirmValue;
                nextSenderState = RESYNC;
...
static void bufferMspResponse(uint8_t *payload) {
    getCrsfMspFrame(tlmBuffer, payload);
}
...
bool getNextTelemetryPayload(uint8_t *nextPayloadSize, uint8_t **payloadData)
{
    ...
    if (deviceInfoReplyPending) {
        ...
        *nextPayloadSize = CRSF_FRAME_SIZE_MAX;
        *payloadData = tlmBuffer;
        mspReplyPending = false;
        return true;
    } 
